...
def rank_dataset(dataset_folder):
  ...
  for folder in os.listdir(dataset_folder):
    ...
    if os.path.isdir(folder_path):
      ...
      df_ranked['amount_sent_rank'] = df_ranked['amount_sent'].rank(ascending=False, method='first')
      df_ranked['average_bandwidth_rank'] = df_ranked['average_bandwidth'].rank(ascending=False, method='first')
      df_ranked['total_rank'] = df_ranked[['duration_rank', 'amount_sent_rank', 'average_bandwidth_rank']].sum(axis=1)
      df_ranked['order'] = df_ranked['total_rank'].rank(method='first', ascending=True)
      # sorted_df = df_ranked.sort_values(by='total_rank')
      df_ranked.drop(['duration_rank', 'amount_sent_rank', 'average_bandwidth_rank'], axis=1, inplace=True)
